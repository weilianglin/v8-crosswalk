        ...
        if (key->value()->IsInternalizedString()) {
          if (property->emit_store()) {
            ...
            if (map.is_null()) {
              ...
              PropertyAccessInfo info(this, STORE, ToType(map), name, map->instance_type());
  ...
  if (GetJSObjectFieldAccess(&access)) {
    for (int i = 1; i < types->length(); ++i) {
      PropertyAccessInfo test_info(
          builder_, access_type_, ToType(types->at(i)), name_, types->at(i)->instance_type());
...
void HOptimizedGraphBuilder::HandlePolymorphicNamedFieldAccess(
  ...
  for (int i = 0; i < types->length() && count < kMaxLoadPolymorphism; ++i) {
    PropertyAccessInfo info(this, access_type, ToType(types->at(i)), name, types->at(i)->instance_type());
...
HInstruction* HOptimizedGraphBuilder::BuildNamedAccess(
  ...
  if (types->length() > 0) {
    PropertyAccessInfo info(this, access, ToType(types->first()), name, types->first()->instance_type());
...
void HOptimizedGraphBuilder::HandlePolymorphicCallNamed(
  ...
  for (int i = 0;
       ...
       ++i) {
    PropertyAccessInfo info(this, LOAD, ToType(types->at(i)), name, types->at(i)->instance_type());
    if (info.CanAccessMonomorphic() &&
...
void HOptimizedGraphBuilder::VisitCall(Call* expr) {
  ...
  if (prop != NULL) {
    ...
    if (prop->key()->IsPropertyName() && types->length() > 0) {
      ...
      PropertyAccessInfo info(this, LOAD, ToType(types->first()), name, types->first()->instance_type());
      if (!info.CanAccessAsMonomorphic(types)) {
